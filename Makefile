...
start/docker/db: ## Start database container
	...
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env up -d server-json-api-postgres server-json-api-pgweb

.PHONY: stop/docker/db
stop/docker/db: ## Stop database container
	@echo "🛑 Stopping database (Docker)..."
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env stop server-json-api-postgres server-json-api-pgweb

.PHONY: start/cache
start/docker/cache: ## Start cache container
	@echo "▶️ Starting cache (Docker)..."
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env up -d server-json-api-redis server-json-api-redis-commander

.PHONY: stop/cache
stop/docker/cache: ## Stop cache container
	@echo "🛑 Stopping cache (Docker)..."
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env stop server-json-api-redis server-json-api-redis-commander
